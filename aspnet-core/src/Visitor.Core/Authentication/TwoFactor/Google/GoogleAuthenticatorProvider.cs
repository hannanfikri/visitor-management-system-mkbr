...
namespace Visitor.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : VisitorServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("Visitor", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
