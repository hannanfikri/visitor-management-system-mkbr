...
using Visitor.Identity;
using Visitor.Web.Chat.SignalR;
using Visitor.Web.Common;
using Swashbuckle.AspNetCore.Swagger;
using Visitor.Web.IdentityServer;
using Visitor.Web.Swagger;
using Stripe;
...
using Visitor.Schemas;
using Visitor.Web.HealthCheck;
...
namespace Visitor.Web.Startup
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            return services.AddAbp<VisitorWebHostModule>(options =>
            {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            if (WebConsts.SwaggerUiEnabled)
            {
                ...
                {
                    options.SwaggerEndpoint(_appConfiguration["App:SwaggerEndPoint"], "Visitor API V1");
                    options.IndexStream = () => Assembly.GetExecutingAssembly()
                        .GetManifestResourceStream("Visitor.Web.wwwroot.swagger.ui.index.html");
        ...
        private void ConfigureSwagger(IServiceCollection services)
        {
            ...
            {
                options.SwaggerDoc("v1", new OpenApiInfo() {Title = "Visitor API", Version = "v1"});
                ...
                if (canShowSummaries)
                {
                    ...
                    var applicationXml = $"Visitor.Application.xml";
                    var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                    options.IncludeXmlComments(applicationXmlPath);

                    var webCoreXmlFile = $"Visitor.Web.Core.xml";
